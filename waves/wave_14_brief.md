...
## TASK 9 — NodeType normalization + template: action + interview: action

**Goal:** 3 related features for AI-friendly node creation:
1. NodeType input normalize (case-insensitive)
2. `template:` action — AI biết required/optional edges per type
3. `interview:` action — AI self-interview để khai đủ relationships

---

### Part A — NodeType normalization

**File to modify:** `gobp/mcp/dispatcher.py`

Add `_normalize_type()` after imports:

```python
# Canonical NodeType mapping — case-insensitive input → PascalCase output
_TYPE_CANONICAL: dict[str, str] = {
    "node": "Node", "idea": "Idea", "decision": "Decision",
    "session": "Session", "document": "Document", "lesson": "Lesson",
    "concept": "Concept", "testkind": "TestKind", "testcase": "TestCase",
    "engine": "Engine", "flow": "Flow", "entity": "Entity",
    "feature": "Feature", "invariant": "Invariant", "screen": "Screen",
    "apiendpoint": "APIEndpoint", "repository": "Repository", "wave": "Wave",
}
...
def _normalize_type(raw: str) -> str:
    ...
    """
    return _TYPE_CANONICAL.get(raw.lower(), raw)
```

Apply in `parse_query()` — normalize node_type after extraction:

```python
    # After extracting node_type from action_part:
    node_type = _normalize_type(node_type) if node_type else ""
```

Apply in `dispatch()` — normalize type in create/update/upsert handlers:
...
### Part B — template: action

**File to modify:** `gobp/mcp/tools/read.py`

Add `_NODE_EDGE_REQUIREMENTS` dict and `node_template()` function:

```python
# Required and optional edges per NodeType
_NODE_EDGE_REQUIREMENTS: dict[str, dict] = {
    "Flow": {
        "required_edges": [
            {"type": "implements", "target": "Protocol or Node", "description": "Protocol này Flow thực hiện"},
            {"type": "references", "target": "Document", "description": "DOC source của Flow"},
        ],
        "optional_edges": [
            {"type": "depends_on", "target": "Entity or Node", "description": "Dependencies"},
            {"type": "tested_by", "target": "TestCase", "description": "Test coverage"},
        ],
    },
    "Engine": {
        "required_edges": [
            {"type": "implements", "target": "Flow or Node", "description": "Flow Engine này phục vụ"},
        ],
        "optional_edges": [
            {"type": "depends_on", "target": "Entity", "description": "Entity Engine cần"},
            {"type": "references", "target": "Document", "description": "Spec document"},
        ],
    },
    "Entity": {
        "required_edges": [
            {"type": "references", "target": "Document", "description": "DOC định nghĩa Entity"},
        ],
        "optional_edges": [
            {"type": "relates_to", "target": "Entity", "description": "Entities liên quan"},
        ],
    },
    "Feature": {
        "required_edges": [
            {"type": "implements", "target": "Flow or Node", "description": "Flow Feature này thuộc về"},
        ],
        "optional_edges": [
            {"type": "references", "target": "Document", "description": "Spec source"},
            {"type": "depends_on", "target": "Entity or Feature", "description": "Dependencies"},
            {"type": "tested_by", "target": "TestCase", "description": "Test coverage"},
        ],
    },
    "Decision": {
        "required_edges": [],
        "optional_edges": [
            {"type": "references", "target": "Document", "description": "Source document"},
            {"type": "supersedes", "target": "Decision", "description": "Decision cũ bị thay thế"},
        ],
    },
    "Document": {
        "required_edges": [],
        "optional_edges": [
            {"type": "references", "target": "Document", "description": "Docs liên quan"},
        ],
    },
    "TestCase": {
        "required_edges": [
            {"type": "covers", "target": "Node or Flow or Feature", "description": "Node này test covers"},
            {"type": "of_kind", "target": "TestKind", "description": "Loại test"},
        ],
        "optional_edges": [],
    },
}
...
def node_template(
    ...
    """
    node_type = args.get("query", args.get("node_type", ""))
    if not node_type:
        # Return all templates
        return {
            "ok": True,
            "templates": {k: v for k, v in _NODE_EDGE_REQUIREMENTS.items()},
            "hint": "gobp(query='template: Flow') to see Flow-specific template",
        }

    reqs = _NODE_EDGE_REQUIREMENTS.get(node_type, {})
    required = reqs.get("required_edges", [])
    optional = reqs.get("optional_edges", [])
    ...
    for edge in required:
        examples.append(
            f"gobp(query=\"edge: {node_type.lower()}:my_id --{edge['type']}--> target:id\")"
            f"  # {edge['description']}"
        )
    ...
    return {
        ...
        "hint": (
            ...
            f"Use interview: to be guided through all relationships."
        ) if required else f"No required edges for {node_type}.",
    }
```
...
```python
            ...
            node_type_arg = _normalize_type(
                params.get("query") or params.get("node_type") or node_type or ""
            )
...
### Part C — interview: action

**File to modify:** `gobp/mcp/tools/read.py`

Add `node_interview()` function:
...
def node_interview(
    ...
    """
    node_id = args.get("node_id") or args.get("query", "")
    if not node_id:
    ...
    reqs = _NODE_EDGE_REQUIREMENTS.get(node_type, {})
    required_edges = reqs.get("required_edges", [])
    optional_edges = reqs.get("optional_edges", [])

    # Check which edges already declared
    existing_edge_types = {
        e.get("type") for e in index.get_edges_from(node_id)
    }
    answered = set(args.get("answered", [])) | existing_edge_types

    # Build questions for undeclared edges
    questions = []
    ...
    for edge in required_edges:
        if edge["type"] not in answered:
            questions.append({
                "edge_type": edge["type"],
                "required": True,
                "question": f"[REQUIRED] {node.get('name', node_id)} {edge['type']} which {edge['target']}?",
                "description": edge["description"],
                "example": f"gobp(query=\"edge: {node_id} --{edge['type']}--> target:id\")",
            })
    ...
    for edge in optional_edges:
        if edge["type"] not in answered:
            questions.append({
                "edge_type": edge["type"],
                "required": False,
                "question": f"[OPTIONAL] Does {node.get('name', node_id)} {edge['type']} any {edge['target']}?",
                "description": edge["description"],
                "example": f"gobp(query=\"edge: {node_id} --{edge['type']}--> target:id\")",
            })

    # Final question — catch-all
    ...
    return {
        ...
        "answered_edge_types": list(answered),
        ...
        "hint": (
            ...
            if questions else
            f"All relationships declared for {node.get('name', node_id)}. ✓"
        ),
    }
```
...
```python
            ...
            node_id = params.get("node_id") or params.get("query") or ""
            answered = params.get("answered", "")
            answered_list = [a.strip() for a in answered.split(",")] if answered else []
            args = {"node_id": node_id, "answered": answered_list}
            result = tools_read.node_interview(index, project_root, args)
```
...
"interview: node:x answered='implements,references'": "Continue interview, skip declared edges",
```

**Acceptance criteria:**
- `_normalize_type("DeciSion")` → `"Decision"`
- `_normalize_type("flow")` → `"Flow"`
- `_normalize_type("UNKNOWN")` → `"UNKNOWN"` (passthrough)
- `template: Flow` returns required_edges with implements + references
- `template:` (no arg) returns all templates
- `interview: node:x` returns questions for undeclared edge types
- Questions marked required vs optional
- `completion_pct` increases as edges are declared
- `done: true` when all required edges declared + catch-all asked

**Commit message:**
```
Wave 14 Task 9: NodeType normalization + template: + interview: actions

- dispatcher.py: _normalize_type() case-insensitive NodeType normalization
- _TYPE_CANONICAL: maps lowercase → PascalCase for all 18 types
- read.py: _NODE_EDGE_REQUIREMENTS per NodeType
- read.py: node_template() — required + optional edges per type
- read.py: node_interview() — guided relationship declaration
- dispatcher.py: template: + interview: actions
- PROTOCOL_GUIDE: 4 new entries
```
...
D:/GoBP/venv/Scripts/python.exe -c "
from gobp.mcp.dispatcher import _normalize_type
cases = [('decision','Decision'),('FLOW','Flow'),('DeciSion','Decision'),('unknown','unknown')]
for raw, expected in cases:
    result = _normalize_type(raw)
    assert result == expected, f'{raw!r} → {result!r} != {expected!r}'
...
async def verify():
    r = await dispatch('template: Flow', index, root)
    print('Template Flow:', r.get('required_count'), 'required,', r.get('optional_count'), 'optional')
    r2 = await dispatch('version:', index, root)
    print('Protocol:', r2.get('protocol_version'))
...
Execute ALL 9 tasks sequentially.
R9: all 326 existing tests must pass after every task.
...
- Task 6: smoke test passed, 326 tests passing
- Task 7: test_wave14.py exists, ~20 tests passing
- Task 8: 340+ tests passing, MCP_TOOLS.md updated, CHANGELOG updated
- Task 9: interview: action in dispatcher, _get_interview_questions() in read.py
          NodeType normalized via _normalize_type() in dispatcher
          template: action returns required + optional edges per type

Expected: 340+ tests. Report WAVE 14 AUDIT COMPLETE.
```
...
Wave 8B — MIHOS import (ALL tools now ready)
  ...
  template: Flow → see required edges before creating
  interview: node:x → guided relationship declaration
```
...
*Author: CTO Chat (Claude Sonnet 4.6)*
*Date: 2026-04-15*
*v2.0: Added Task 9 — NodeType normalization + template: + interview: actions*

"""GoBP file-backed mutator — create/update/delete on YAML nodes and edges.
...
This module does NOT manage the in-memory GraphIndex. Callers are
responsible for reloading the index after mutation if they need
fresh state. (Wave G: renamed from ``mutator.py``; v3 PG path is
:mod:`gobp.core.mutator_v3`.)
"""
...
def coerce_and_validate_node(
    ...
    if sn == "gobp_core_v3":
        ...
        v3 = ValidatorV3()
        nt = schema.get("node_types") if isinstance(schema.get("node_types"), dict) else {}
        fixed = v3.auto_fix(dict(node), node_types=nt)
        errs = v3.validate(fixed)
        node.clear()
        node.update(fixed)
        if errs:
            return ValidationResult(ok=False, errors=errs)
...
def create_node(
    ...
    try:
        from gobp.mcp.pg_sync import maybe_upsert_node_v3

        maybe_upsert_node_v3(gobp_root, dict(node))
    except Exception:
        pass
...
def create_nodes_batch(
    ...
    try:
        ...
        for n in nodes:
            maybe_upsert_node_v3(gobp_root, dict(n))
    except Exception:
        pass
...
def create_edge(
    ...
    try:
        from gobp.mcp.pg_sync import maybe_upsert_edge_v3

        maybe_upsert_edge_v3(gobp_root, dict(edge))
    except Exception:
        pass
...
def append_edges_batch(
    ...
    try:
        ...
        for e in appended:
            maybe_upsert_edge_v3(gobp_root, dict(e))
    except Exception:
        pass

...
def extract_candidates(text: str) -> list[tuple[str, str]]:
    ...
    for line in text.splitlines():
        ...
        if not s:
            continue
        # markdown headings likely hold intent
        if s.startswith('## TASK') or s.startswith('###'):
            out.append((s, 'task_heading'))
        # goal lines also describe actionable capabilities
...
def classify_type(label: str) -> str:
    ...
    if 'invariant' in l:
        return 'Invariant'
    if 'feature' in l or 'layout' in l or 'viewer' in l or 'migration' in l or 'id' in l:
        return 'Feature'
    return 'Node'


async def safe_dispatch(q: str):
    idx = GraphIndex.load_from_disk(ROOT)
    return await dispatch(q, idx, ROOT)


async def main() -> None:
    sess = await safe_dispatch("session:start actor='cursor' goal='materialize wave docs theory into typed nodes and traceability edges'")
    sid = sess['session_id']
    ...
    for fp in WAVES:
        # find existing document node imported from this source path
        ...
        if wave_id and doc_id:
            e = await safe_dispatch(
                f"edge: {wave_id} --references--> {doc_id} reason='wave node references wave source document'"
            )
            if e.get('ok'):
                edge_ok += 1
        ...
        for raw, source_kind in cands:
            # sanitize compact title
            title = re.sub(r"\*+", '', raw)
